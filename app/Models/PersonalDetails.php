...
class PersonalDetails extends Model
{
           ...
           if($request->input('principle_residence') !== null || $request->input('cash_everyday') !== null ){
            ...
            foreach ($request->input('personalDebtRatesCreditCard') as $value) {
                  ...
                  if($request->input('total_income_client_annual') !== null && $request->input('total_income_partner_annual') !== null)
                {
                    ...
                    $current_position->investment_portfolio_assets_long_term_savings = $request->input('investment_portfolio_assets_long_term_savings');
                    $current_position->investment_portfolio_assets_shares = $request->input('investment_portfolio_assets_shares');
                    $current_position->investment_portfolio_assets_business_net_value = $request->input('investment_portfolio_assets_business_net_value');
                    $current_position->investment_portfolio_assets_existing_investment_property = $request->input('investment_portfolio_assets_existing_investment_property');
                    $current_position->investment_portfolio_assets_mortgage = $request->input('investment_portfolio_assets_mortgage');
                    $current_position->investment_portfolio_net_financial_assets= $request->input('investment_portfolio_net_financial_assets');
                    ...
                    $financial_independance = new SheetFinancialIndependance;
                    $financial_independance->details_id   = $request->input('details_id');
                    $financial_independance->gross_household_income_per_annum = $request->input('gross_household_income_per_annum');
                    $financial_independance->desired_current_income_required_retirement = $request->input('desired_current_income_required_retirement');
                    $financial_independance->annual_gross_houshold_income_required_in_retirement = $request->input('annual_gross_houshold_income_required_in_retirement');
                    $financial_independance->weekly_gross_household_income = $request->input('weekly_gross_household_income');
                    $financial_independance->age_this_year = $request->input('age_this_year');
                    $financial_independance->prefer_retirement_age = $request->input('prefer_retirement_age');
                    $financial_independance->years_to_achieve_financial_independence = $request->input('years_to_achieve_financial_independence');
                    $financial_independance->net_financial_assets = $request->input('net_financial_assets');
                    $financial_independance->total_investment_portfolio_required = $request->input('total_investment_portfolio_required');
                    $financial_independance->total_annual_household_income_retirement = $request->input('total_annual_household_income_retirement');
                    $financial_independance->equivalent_value_of_annual_household = $request->input('equivalent_value_of_annual_household');
                    $financial_independance->your_current_net_financial_assets_value = $request->input('your_current_net_financial_assets_value');
                    $financial_independance->annual_increase_in_net_financial_assets = $request->input('annual_increase_in_net_financial_assets');
                    $financial_independance->monthly_increase_in_net_financial_assets = $request->input('monthly_increase_in_net_financial_assets');
                ...
                if($request->input('ip_1') !== null && $request->input('retired_age_ip1') !== null && $request->input('property_purchase_ip1') && $request->input('lvr_ip1') && $request->input('other_purchased_cost_ip1')){
                    ...
                    $proposednewIp->stamp_duty_ip1   = $request->input('stamp_duty_ip1');
                    $proposednewIp->stamp_duty_ip2   = $request->input('stamp_duty_ip2');
                    $proposednewIp->stamp_duty_ip3   = $request->input('stamp_duty_ip3');
                    $proposednewIp->stamp_duty_ip4   = $request->input('stamp_duty_ip4');
                    $proposednewIp->stamp_duty_ip5   = $request->input('stamp_duty_ip5');
                    $proposednewIp->stamp_duty_ip6   = $request->input('stamp_duty_ip6');
                    $proposednewIp->stamp_duty_ip7   = $request->input('stamp_duty_ip7');
  ...
  try {
            ...
            $updateFI = FinancialIndependence::where('details_id',$id)->update([
                ...
                $verifyIncomeData = Income::where('details_id',$id)->first();
                if($verifyIncomeData !== null)
                {
                ...
                $verifySuperannuation = Superannuation::where('details_id',$id)->first();
                if($verifySuperannuation !== null){
                ...
                $verifyInvestmentDebtRates = Investment_Debt_Rates::where('details_id',$id)->first();
                if($verifyInvestmentDebtRates !== null){
                ...
                foreach ($request->input('row', []) as $value) {
                    if (!isset($value['non_investment_owner'])) {
                        continue;
                        }
                $investmentId = !empty($value['investment_id']) ? $value['investment_id'] : null;
                $verifyInvestmentPropertyAsset = null;
                    if ($investmentId !== null) {
                        $verifyInvestmentPropertyAsset = InvestmentPropertyAsset::where('id', $investmentId)->first();
                     }

   
                $data = [
                    'investment_property' => $value['non_investment_owner'],
                    'client_percentage'   => $value['client_percentage'] ?? 0,
                    'partner_percentage'  => $value['partner_percentage'] ?? 0,
                    'market_value'        => $value['market_value'] ?? 0,
                    'client'              => $value['client'] ?? 0,
                    'partner'             => $value['partner'] ?? 0,
                ];

                if ($verifyInvestmentPropertyAsset !== null) {
    
                    $verifyInvestmentPropertyAsset->update($data);
                } 
                else {
   
                    $investment = new InvestmentPropertyAsset;
                    $investment->details_id = $request->input('details_id'); 
                    $investment->investment_property = $data['investment_property'];
                    $investment->client_percentage   = $data['client_percentage'];
                    $investment->partner_percentage  = $data['partner_percentage'];
                    $investment->market_value        = $data['market_value'];
                    $investment->client              = $data['client'];
                    $investment->partner             = $data['partner'];
                    $investment->encoded_by   = $request->input('encoded_by');
                    $investment->date_encoded = \Carbon\Carbon::now()->toDateString(); 
                        $investment->save();
                    }
                }
            ...
            //     $verifyOtherPersonalNonInvestment = Other_Personal_Assets_Non_Invesmtment::where('id',$value['others_id'])->first();
            //     if($verifyOtherPersonalNonInvestment !== null){
            ...
            // }
            foreach ($request->input('noninvestmentasset', []) as $value) {
                if (!isset($value['other_personal_asset'])) {
                    continue;
                    }
                $assetId = !empty($value['others_id']) ? $value['others_id'] : null;
                $verifyOtherPersonalNonInvestment = null;
                 if ($assetId !== null) {
                    $verifyOtherPersonalNonInvestment = Other_Personal_Assets_Non_Invesmtment::where('id', $assetId)->first();
                    }

                 $data = [
                    'other_personal_asset'                    => $value['other_personal_asset'],
                    'non_investment_asset_client_percentage'  => $value['non_investment_asset_client_percentage'] ?? 0,
                    'non_investment_asset_partner_percentage' => $value['non_investment_asset_partner_percentage'] ?? 0,
                    'non_investment_asset_market_value'      => $value['non_investment_asset_market_value'] ?? 0,
                    'non_investment_asset_client'            => $value['non_investment_asset_client'] ?? 0,
                    'non_investment_asset_partner'           => $value['non_investment_asset_partner'] ?? 0,
                    ];

                if ($verifyOtherPersonalNonInvestment !== null) {
                    $verifyOtherPersonalNonInvestment->update($data);
                    } 
                    else {
        
                $non_investment = new Other_Personal_Assets_Non_Invesmtment;
                $non_investment->details_id = $request->input('details_id'); 
                $non_investment->other_personal_asset                    = $data['other_personal_asset'];
                $non_investment->non_investment_asset_client_percentage  = $data['non_investment_asset_client_percentage'];
                $non_investment->non_investment_asset_partner_percentage = $data['non_investment_asset_partner_percentage'];
                $non_investment->non_investment_asset_market_value      = $data['non_investment_asset_market_value'];
                $non_investment->non_investment_asset_client            = $data['non_investment_asset_client'];
                $non_investment->non_investment_asset_partner           = $data['non_investment_asset_partner'];
                $non_investment->encoded_by   = $request->input('encoded_by');
                $non_investment->date_encoded = \Carbon\Carbon::now()->toDateString();
                $non_investment->save();
                }
        }


                $verifyNonInvestmentAssets = Non_Investment_Assets::where('details_id',$id)->first();
                if($verifyNonInvestmentAssets !== null)
                {
                ...
                $verifyInvestmentAsset = Investment_Asset::where('details_id',$id)->first();
                if($verifyInvestmentAsset !== null)
                {
                ...
                if($verifyLiabilitiesNonInvestment !== null)
                {
                 ...
                 $verifyProposedNewIP = ProposedNewInvestmentProp::where('details_id',$id)->first();
                 if($verifyProposedNewIP !== null)
                    {
                          $updateProposedNewIP = ProposedNewInvestmentProp::where('details_id',$id)->update([
                            'ip_1' => $request->input('ip_1'),
                            'ip2_date' => $request->input('ip2_date'),
                            'ip3_date' => $request->input('ip3_date'),
                            'ip4_date' => $request->input('ip4_date'),
                            'ip5_date' => $request->input('ip5_date'),
                            'ip6_date' => $request->input('ip6_date'),
                            'ip7_date' => $request->input('ip7_date'),
                            'months_last_acq_ip1' => $request->input('months_last_acq_ip1'),
                            'months_last_acq_ip2' => $request->input('months_last_acq_ip2'),
                            'months_last_acq_ip3' => $request->input('months_last_acq_ip3'),
                            'months_last_acq_ip4' => $request->input('months_last_acq_ip4'),
                            'months_last_acq_ip5' => $request->input('months_last_acq_ip5'),
                            'months_last_acq_ip6' => $request->input('months_last_acq_ip6'),
                            'months_last_acq_ip7' => $request->input('months_last_acq_ip7'),
                            'retired_age_ip1' => $request->input('retired_age_ip1'),
                            'retired_age_ip2' => $request->input('retired_age_ip2'),
                            'retired_age_ip3' => $request->input('retired_age_ip3'),
                            'retired_age_ip4' => $request->input('retired_age_ip4'),
                            'retired_age_ip5' => $request->input('retired_age_ip5'),
                            'retired_age_ip6' => $request->input('retired_age_ip6'),
                            'retired_age_ip7' => $request->input('retired_age_ip7'),
                            'property_purchase_ip1' => $request->input('property_purchase_ip1'),
                            'property_purchase_ip2' => $request->input('property_purchase_ip2'),
                            'property_purchase_ip3' => $request->input('property_purchase_ip3'),
                            'property_purchase_ip4' => $request->input('property_purchase_ip4'),
                            'property_purchase_ip5' => $request->input('property_purchase_ip5'),
                            'property_purchase_ip6' => $request->input('property_purchase_ip6'),
                            'property_purchase_ip7' => $request->input('property_purchase_ip7'),
                            'stamp_duty_ip1' => $request->input('stamp_duty_ip1'),
                            'stamp_duty_ip2' => $request->input('stamp_duty_ip2'),
                            'stamp_duty_ip3' => $request->input('stamp_duty_ip3'),
                            'stamp_duty_ip4' => $request->input('stamp_duty_ip4'),
                            'stamp_duty_ip5' => $request->input('stamp_duty_ip5'),
                            'stamp_duty_ip6' => $request->input('stamp_duty_ip6'),
                            'stamp_duty_ip7' => $request->input('stamp_duty_ip7'),
                            'lvr_ip1' => $request->input('lvr_ip1'),
                            'lvr_ip2' => $request->input('lvr_ip2'),
                            'lvr_ip3' => $request->input('lvr_ip3'),
                            'lvr_ip4' => $request->input('lvr_ip4'),
                            'lvr_ip5' => $request->input('lvr_ip5'),
                            'lvr_ip6' => $request->input('lvr_ip6'),
                            'lvr_ip7' => $request->input('lvr_ip7'),
                            'other_purchased_cost_ip1' => $request->input('other_purchased_cost_ip1'),
                            'other_purchased_cost_ip2' => $request->input('other_purchased_cost_ip2'),
                            'other_purchased_cost_ip3' => $request->input('other_purchased_cost_ip3'),
                            'other_purchased_cost_ip4' => $request->input('other_purchased_cost_ip4'),
                            'other_purchased_cost_ip5' => $request->input('other_purchased_cost_ip5'),
                            'other_purchased_cost_ip6' => $request->input('other_purchased_cost_ip6'),
                            'other_purchased_cost_ip7' => $request->input('other_purchased_cost_ip7'),
                            'loan_value_based_ip1' => $request->input('loan_value_based_ip1'),
                            'loan_value_based_ip2' => $request->input('loan_value_based_ip2'),
                            'loan_value_based_ip3' => $request->input('loan_value_based_ip3'),
                            'loan_value_based_ip4' => $request->input('loan_value_based_ip4'),
                            'loan_value_based_ip5' => $request->input('loan_value_based_ip5'),
                            'loan_value_based_ip6' => $request->input('loan_value_based_ip6'),
                            'loan_value_based_ip7' => $request->input('loan_value_based_ip7'),
                            'total_loan_value_ip1' => $request->input('total_loan_value_ip1'),
                            'total_loan_value_ip2' => $request->input('total_loan_value_ip2'),
                            'total_loan_value_ip3' => $request->input('total_loan_value_ip3'),
                            'total_loan_value_ip4' => $request->input('total_loan_value_ip4'),
                            'total_loan_value_ip5' => $request->input('total_loan_value_ip5'),
                            'total_loan_value_ip6' => $request->input('total_loan_value_ip6'),
                            'total_loan_value_ip7' => $request->input('total_loan_value_ip7'),
                            'estimated_rent_ip1' => $request->input('estimated_rent_ip1'),
                            'estimated_rent_ip2' => $request->input('estimated_rent_ip2'),
                            'estimated_rent_ip3' => $request->input('estimated_rent_ip3'),
                            'estimated_rent_ip4' => $request->input('estimated_rent_ip4'),
                            'estimated_rent_ip5' => $request->input('estimated_rent_ip5'),
                            'estimated_rent_ip6' => $request->input('estimated_rent_ip6'),
                            'estimated_rent_ip7' => $request->input('estimated_rent_ip7'),

                            'estimated_outgoing_ip1' => $request->input('estimated_outgoing_ip1'),
                            'estimated_outgoing_ip2' => $request->input('estimated_outgoing_ip2'),
                            'estimated_outgoing_ip3' => $request->input('estimated_outgoing_ip3'),
                            'estimated_outgoing_ip4' => $request->input('estimated_outgoing_ip4'),
                            'estimated_outgoing_ip5' => $request->input('estimated_outgoing_ip5'),
                            'estimated_outgoing_ip6' => $request->input('estimated_outgoing_ip6'),
                            'estimated_outgoing_ip7' => $request->input('estimated_outgoing_ip7'),
                            'estimated_annual_interest_ip1' => $request->input('estimated_annual_interest_ip1'),
                            'estimated_annual_interest_ip2' => $request->input('estimated_annual_interest_ip2'),
                            'estimated_annual_interest_ip3' => $request->input('estimated_annual_interest_ip3'),
                            'estimated_annual_interest_ip4' => $request->input('estimated_annual_interest_ip4'),
                            'estimated_annual_interest_ip5' => $request->input('estimated_annual_interest_ip5'),
                            'estimated_annual_interest_ip6' => $request->input('estimated_annual_interest_ip6'),
                            'estimated_annual_interest_ip7' => $request->input('estimated_annual_interest_ip7'),
                ...
                foreach ($request->input('debt', []) as $value) {
                 if (!isset($value['other_debt'])) {
                    ...
                    $debtId = !empty($value['other_debt_id']) ? $value['other_debt_id'] : null;
                    $verifyOtherDebt = null;
                    if ($debtId !== null) {
                        $verifyOtherDebt = Other_Debt::where('id', $debtId)->first();
                    }

                $data = [
                    'other_debt'                    => $value['other_debt'],
                    'other_debt_client_percentage'  => $value['other_debt_client_percentage'] ?? 0,
                    'other_debt_partner_percentage' => $value['other_debt_partner_percentage'] ?? 0,
                    'other_debt_market_value'       => $value['other_debt_market_value'] ?? 0,
                    'other_debt_client'             => $value['other_debt_client'] ?? 0,
                    'other_debt_parnter'            => $value['other_debt_parnter'] ?? 0, 
                    ];

                if ($verifyOtherDebt !== null) {
                        $verifyOtherDebt->update($data);
                        } 
                else {
                    $other_debt = new Other_Debt; 
                    $other_debt->details_id = $request->input('details_id'); 
                    $other_debt->other_debt                    = $data['other_debt'];
                    $other_debt->other_debt_client_percentage  = $data['other_debt_client_percentage'];
                    $other_debt->other_debt_partner_percentage = $data['other_debt_partner_percentage'];
                    $other_debt->other_debt_market_value       = $data['other_debt_market_value'];
                    $other_debt->other_debt_client             = $data['other_debt_client'];
                    $other_debt->other_debt_parnter            = $data['other_debt_parnter'];
                    $other_debt->encoded_by   = $request->input('encoded_by');
                    $other_debt->date_encoded = \Carbon\Carbon::now()->toDateString();
                    $other_debt->save(); 
                    }
                }
                ...
                foreach ($request->input('creditcard', []) as $value) {
                    if (!isset($value['credit_card'])) {
                        continue;
                     }
                    $cardId = !empty($value['credit_card_id']) ? $value['credit_card_id'] : null;
                    $verifyCreditCard = null;
                    if ($cardId !== null) {
                        $verifyCreditCard = Credit_Card::where('id', $cardId)->first();
                        }
                    $data = [
                        'credit_card'                    => $value['credit_card'],
                        'credit_card_client_percentage'  => $value['credit_card_client_percentage'] ?? 0,
                        'credit_card_partner_percentage' => $value['credit_card_partner_percentage'] ?? 0,
                        'credit_card_market_value'      => $value['credit_card_market_value'] ?? 0,
                        'credit_card_client'            => $value['credit_card_client'] ?? 0,
                        'credit_card_partner'           => $value['credit_card_partner'] ?? 0,
                        ];

                    if ($verifyCreditCard !== null) {
                        $verifyCreditCard->update($data);
                        } 
                    else {
                    ...
                    $credit_card->credit_card_client_percentage  = $data['credit_card_client_percentage'];
                    $credit_card->credit_card_partner_percentage = $data['credit_card_partner_percentage'];
                    $credit_card->credit_card_market_value      = $data['credit_card_market_value'];
                    $credit_card->credit_card_client            = $data['credit_card_client'];
                    $credit_card->credit_card_partner           = $data['credit_card_partner']; 
                    $credit_card->encoded_by   = $request->input('encoded_by');
                    $credit_card->date_encoded = \Carbon\Carbon::now()->toDateString();
                    $credit_card->save(); 
                    }
                }
            ...
            foreach ($request->input('mortgageInvestment', []) as $value) {
                if (!isset($value['mortgage_investment'])) {
                    continue;
                }
            $mortgageId = !empty($value['mortgage_investment_id']) ? $value['mortgage_investment_id'] : null;
            $verifyMortgageInvestmentProperty = null;
            if ($mortgageId !== null) {
                $verifyMortgageInvestmentProperty = Mortgage_Investment_Property::where('id', $mortgageId)->first();
            }

            $data = [
                'mortgage_investment'                    => $value['mortgage_investment'],
                'mortgage_investment_client_percentage'  => $value['mortgage_investment_client_percentage'] ?? 0,
                'mortgage_investment_partner_percentage' => $value['mortgage_investment_partner_percentage'] ?? 0,
                'mortgage_investment_market_value'      => $value['mortgage_investment_market_value'] ?? 0,
                'mortgage_investment_client'            => $value['mortgage_investment_client'] ?? 0,
                'mortgage_investment_partner'           => $value['mortgage_investment_partner'] ?? 0,
                ];

            if ($verifyMortgageInvestmentProperty !== null) {
                $verifyMortgageInvestmentProperty->update($data);
            } 
            else {
        
                $mortgage = new Mortgage_Investment_Property;
                $mortgage->details_id = $request->input('details_id'); 
                $mortgage->mortgage_investment                    = $data['mortgage_investment'];
                $mortgage->mortgage_investment_client_percentage  = $data['mortgage_investment_client_percentage'];
                $mortgage->mortgage_investment_partner_percentage = $data['mortgage_investment_partner_percentage'];
                $mortgage->mortgage_investment_market_value      = $data['mortgage_investment_market_value'];
                $mortgage->mortgage_investment_client            = $data['mortgage_investment_client'];
                $mortgage->mortgage_investment_partner           = $data['mortgage_investment_partner'];   
                $mortgage->encoded_by   = $request->input('encoded_by');
                $mortgage->date_encoded = \Carbon\Carbon::now()->toDateString();
                $mortgage->save();
                }
            }
            ...
            foreach ($request->input('personalDebtRateOtherDebt', []) as $value) {
                if (!isset($value['personal_debt_rate_other_debts'])) {
                    continue;
                }

            $rateId = !empty($value['debt_rates_other_id']) ? $value['debt_rates_other_id'] : null;
            $verifyPersonalDebtRateOtherDebt = null;
            if ($rateId !== null) {
                $verifyPersonalDebtRateOtherDebt = Personal_Debt_Rate_Other_Debt::where('id', $rateId)->first();
            }


            $data = [
                'personal_debt_rate_other_debts'      => $value['personal_debt_rate_other_debts'],
                'personal_debt_rate_other_debt_years' => $value['personal_debt_rate_other_debt_years'] ?? 0,
            ];

            if ($verifyPersonalDebtRateOtherDebt !== null) {
                $verifyPersonalDebtRateOtherDebt->update($data);
            } 
            else {
                $debt_rates_other_debt = new Personal_Debt_Rate_Other_Debt;
                $debt_rates_other_debt->details_id = $request->input('details_id'); 
                $debt_rates_other_debt->personal_debt_rate_other_debts      = $data['personal_debt_rate_other_debts'];
                $debt_rates_other_debt->personal_debt_rate_other_debt_years = $data['personal_debt_rate_other_debt_years'];
                $debt_rates_other_debt->encoded_by   = $request->input('encoded_by');
                $debt_rates_other_debt->date_encoded = \Carbon\Carbon::now()->toDateString();
                $debt_rates_other_debt->save(); 
            }
        }
            ...
            foreach ($request->input('personalDebtRatesCreditCard', []) as $value) {
                if (!isset($value['personal_debt_rate_credit_card'])) {
                    continue;
                }

                $rateId = !empty($value['debt_rates_credit_card_id']) ? $value['debt_rates_credit_card_id'] : null;
                $verifyPersonalDebtRateCreditCard = null;
                if ($rateId !== null) {
                    $verifyPersonalDebtRateCreditCard = Personal_Debt_Rate_Credit_Card::where('id', $rateId)->first();
                }

                $data = [
                    'personal_debt_rate_credit_card'       => $value['personal_debt_rate_credit_card'],
                    'personal_debt_rate_credit_card_years' => $value['personal_debt_rate_credit_card_years'] ?? 0,
                    ];

                if ($verifyPersonalDebtRateCreditCard !== null) {
                    $verifyPersonalDebtRateCreditCard->update($data);
                } 
                else {
        
                    $debt_rates_credit_card = new Personal_Debt_Rate_Credit_Card;
                    $debt_rates_credit_card->details_id = $request->input('details_id'); 
                    $debt_rates_credit_card->personal_debt_rate_credit_card       = $data['personal_debt_rate_credit_card'];
                    $debt_rates_credit_card->personal_debt_rate_credit_card_years = $data['personal_debt_rate_credit_card_years']; 
                    $debt_rates_credit_card->encoded_by   = $request->input('encoded_by');
                    $debt_rates_credit_card->date_encoded = \Carbon\Carbon::now()->toDateString();
                    $debt_rates_credit_card->save();
                }
            }
                ...
                $verifyCurrentPostion = CurrentPosition::where('details_id',$id)->first();
                if($verifyCurrentPostion !== null ){
                    $updatecurrentposition = CurrentPosition::where('details_id',$id)->update([
                    'gross_anual_income_client' => $request->input('gross_anual_income_client'),
                    'gross_anual_income_partner' => $request->input('gross_anual_income_partner'),
                    'total_houese_hold_income' => $request->input('total_houese_hold_income'),
                    'your_home_mortgage' => $request->input('your_home_mortgage'),
                    'equity_in_your_home' => $request->input('equity_in_your_home'),
                    'investment_portfolio_long_term_savings' => $request->input('investment_portfolio_long_term_savings'),
                    'investment_portfolio_superannuation_client_net_value' => $request->input('investment_portfolio_superannuation_client_net_value'),
                    'investment_portfolio_superannuation_partner_net_value' => $request->input('investment_portfolio_superannuation_partner_net_value'),
                    'investment_portfolio_shares_net_value' => $request->input('investment_portfolio_shares_net_value'),
                    'investment_portfolio_business_net_value' => $request->input('investment_portfolio_business_net_value'),
                    'investment_portfolio_existing_investment_property' => $request->input('investment_portfolio_existing_investment_property'),
                    'investment_portfolio_mortgage' => $request->input('investment_portfolio_mortgage'),
                    'investment_portfolio_total' => $request->input('investment_portfolio_total'),
                    'investment_portfolio_net_position' => $request->input('investment_portfolio_net_position'),
                    'investment_portfolio_repay_mortgage' => $request->input('investment_portfolio_repay_mortgage'),
                    'investment_portfolio_current_net_financial_assets' => $request->input('investment_portfolio_current_net_financial_assets'),
                    'projected_value_of_your_home' => $request->input('projected_value_of_your_home'),
                    'investment_portfolio_assets_superannuation' => $request->input('investment_portfolio_assets_superannuation'),
                    'investment_portfolio_assets_long_term_savings' => $request->input('investment_portfolio_assets_long_term_savings'),
                    'investment_portfolio_assets_shares' => $request->input('investment_portfolio_assets_shares'),
                    'investment_portfolio_assets_business_net_value' => $request->input('investment_portfolio_assets_business_net_value'),
                    'investment_portfolio_assets_existing_investment_property' => $request->input('investment_portfolio_assets_existing_investment_property'),
                    'investment_portfolio_assets_mortgage' => $request->input('investment_portfolio_assets_mortgage'),
                    'investment_portfolio_net_financial_assets' => $request->input('investment_portfolio_net_financial_assets'),
                          ]);
                } else {
                ...
                $verifyfinancialIndependance = SheetFinancialIndependance::where('details_id',$id)->first();
                if($verifyfinancialIndependance !== null)
                    {
                     $updatefinancialIndepenfance = SheetFinancialIndependance::where('details_id',$id)->update([
                    'gross_household_income_per_annum' => $request->input('gross_household_income_per_annum'),
                    'desired_current_income_required_retirement' => $request->input('desired_current_income_required_retirement'),
                    'annual_gross_houshold_income_required_in_retirement' => $request->input('annual_gross_houshold_income_required_in_retirement'),
                    'weekly_gross_household_income' => $request->input('weekly_gross_household_income'),
                    'age_this_year' => $request->input('age_this_year'),
                    'prefer_retirement_age' => $request->input('prefer_retirement_age'),
                    'years_to_achieve_financial_independence' => $request->input('years_to_achieve_financial_independence'),
                    'net_financial_assets' => $request->input('net_financial_assets'),
                    'total_investment_portfolio_required' => $request->input('total_investment_portfolio_required'),
                    'total_annual_household_income_retirement' => $request->input('total_annual_household_income_retirement'),
                    'equivalent_value_of_annual_household' => $request->input('equivalent_value_of_annual_household'),
                    'your_current_net_financial_assets_value' => $request->input('your_current_net_financial_assets_value'),
                    'annual_increase_in_net_financial_assets' => $request->input('annual_increase_in_net_financial_assets'),
                    'monthly_increase_in_net_financial_assets' => $request->input('monthly_increase_in_net_financial_assets'),
                    'weekly_increase_in_net_financial_assets' => $request->input('weekly_increase_in_net_financial_assets'),
                    'current_level_of_income_and_expenses' => $request->input('current_level_of_income_and_expenses'),
                    'total_investment_portfolio_achieve_annual_household_today' => $request->input('total_investment_portfolio_achieve_annual_household_today'),
